...
static void sl_802154_load_addr(uint8_t *buffer, uint8_t buffer_len, size_t *length,
...
static void sl_802154_load_mhr(uint8_t *buffer, uint8_t buffer_len, struct ieee802154_mhr *mhr)
{
...
static int src_match_ext_add(struct silabs_efr32_source_match_data *md, const uint8_t *addr)
{
	...
	memcpy(md->ext_addrs[md->ext_addr_count++], addr, 8);
					       ...
					       bool rx_pan_present)
{
	...
	const struct ieee802154_address *src = rx_pan_present ? &rx_addr_field->plain.addr
							      : &rx_addr_field->comp.addr;

	if (mode == IEEE802154_ADDR_MODE_SHORT) {
		if (out_size - off < IEEE802154_SHORT_ADDR_LENGTH) {
			return 0;
		}
		memcpy(out + off, &src->short_addr, IEEE802154_SHORT_ADDR_LENGTH);
		off += IEEE802154_SHORT_ADDR_LENGTH;
	} else if (mode == IEEE802154_ADDR_MODE_EXTENDED) {
		if (out_size - off < IEEE802154_EXT_ADDR_LENGTH) {
			return 0;
		}
		memcpy(out + off, src->ext_addr, IEEE802154_EXT_ADDR_LENGTH);
		off += IEEE802154_EXT_ADDR_LENGTH;
	} else {
...
static uint8_t construct_enh_ack_mhr(struct silabs_efr32_802154_data *data, bool ie_present)
{
	...
	data->enh_ack_mhr.fs = &enh_ack_fs;
	...
	(void)sl_802154_compose_ack_addr_field(ack_dst_field, sizeof(ack_dst_field),
					       ...
					       ack_pl.dst_pan_present, ack_dst_pan,
					       data->rx_mhr.src_addr, rx_pl.src_pan_present);
	(void)sl_802154_compose_ack_addr_field(ack_src_field, sizeof(ack_src_field),
					       enh_ack_fs.fc.src_addr_mode,
					       ack_pl.src_pan_present, ack_src_pan,
					       data->rx_mhr.dst_addr, rx_pl.dst_pan_present);

	data->enh_ack_mhr.dst_addr = (struct ieee802154_address_field *)ack_dst_field;
	data->enh_ack_mhr.src_addr = (struct ieee802154_address_field *)ack_src_field;
	if (data->rx_mhr.fs->fc.security_enabled) {
		data->enh_ack_mhr.aux_sec = &enh_sec_hdr;
	} else {
		data->enh_ack_mhr.aux_sec = NULL;
	}

	return sl_802154_serialize_mhr(data->enh_ack_buffer + SL_802154_PHR_BYTES,
			       ...
			       uint32_t rx_timestamp, uint8_t *initial_pkt_read_bytes)
{
	...
	uint8_t expected_bytes =
		(SL_802154_PHR_BYTES + IEEE802154_FCF_SEQ_LENGTH + IEEE802154_PAN_ID_LENGTH +
		 IEEE802154_EXT_ADDR_LENGTH + IEEE802154_PAN_ID_LENGTH +
		 IEEE802154_EXT_ADDR_LENGTH + IEEE802154_SECURITY_CF_LENGTH +
		 IEEE802154_SECURITY_FRAME_COUNTER_LENGTH +
		 IEEE802154_KEY_ID_FIELD_SRC_8_INDEX_LENGTH);
	uint8_t max_expected_bytes_with_ie = expected_bytes + OT_ACK_IE_MAX_SIZE;

	__maybe_unused sl_rail_status_t status;
	uint8_t enh_ack_mhr_length;
	uint8_t enh_ack_len;

	*initial_pkt_read_bytes =
		read_initial_pkt_data(data, pkt_info_for_enh_ack, expected_bytes,
				      SL_802154_PHR_BYTES + 2, max_expected_bytes_with_ie);
	...
	if (IS_ENABLED(CONFIG_OPENTHREAD_LINK_METRICS_SUBJECT)) {
		ie_present |= data->ack_ie.link_metrics_header_ie.length > 0;
	}

	enh_ack_mhr_length = construct_enh_ack_mhr(data, ie_present);
	enh_ack_len = SL_802154_PHR_BYTES + enh_ack_mhr_length;

	if (IS_ENABLED(CONFIG_OPENTHREAD_LINK_METRICS_SUBJECT)) {
		/* append Link Metrics IE header to payload */
		if (data->ack_ie.link_metrics_header_ie.length > 0) {
			memcpy(data->enh_ack_buffer + enh_ack_len,
			       &data->ack_ie.link_metrics_header_ie,
			       data->ack_ie.link_metrics_header_ie.length);
			enh_ack_len += data->ack_ie.link_metrics_header_ie.length;
		}
	}
	...
	data->enh_ack_buffer[0] = enh_ack_len;
	status = sl_rail_ieee802154_write_enh_ack(data->radio_data.rail_handle,
...
static void handle_data_request_command(struct silabs_efr32_802154_data *data)
{
	...
	sl_rail_rx_packet_info_t pkt_info;
	sl_rail_status_t status = SL_STATUS_OK;
...
static bool validate_phr(const sl_rail_rx_packet_info_t *pkt_info, uint16_t pkt_length)
{
			 ...
			 sl_rail_rx_packet_details_t *pkt_details, uint16_t pkt_length)
{
	...
	if (!validate_phr(pkt_info, pkt_length)) {
		goto exit;
	}
...
static void handle_rx_pkt(const struct device *dev)
{
	...
	/**
	 * RAIL's packet_bytes includes the (1 or 2 byte) PHY header but not the 2-byte CRC.
	 * We want pkt_length to match the PHY header length so we add 2 for CRC
	...
	sl_802154_load_mhr(data->rx_buffer, pkt_info.packet_bytes - SL_802154_PHR_BYTES,
			   &data->rx_mhr);

	if (pkt_details.is_ack) {
		handle_rx_ack(dev, pkt_length, &pkt_details);
	} else {
		handle_rx_data(dev, pkt_length, &pkt_details);
	}
}
...
static void silabs_efr32_rail_events_callback(sl_rail_handle_t handle, sl_rail_events_t events)
{
	...
	if ((events & SL_RAIL_EVENT_IEEE802154_DATA_REQUEST_COMMAND) != 0ULL) {
		handle_data_request_command(data);
	}

	/* Process TX events */
	if (events & SL_RAIL_EVENTS_TX_COMPLETION) {
		handle_tx_completion(data, events);
	}
...
static int sl_802154_rail_config(const struct device *dev)
{
	...
	sl_rail_events_t events =
		(SL_RAIL_EVENTS_NONE | SL_RAIL_EVENT_RX_ACK_TIMEOUT |
		 SL_RAIL_EVENT_RX_PACKET_RECEIVED | SL_RAIL_EVENTS_TXACK_COMPLETION |
		 SL_RAIL_EVENTS_TX_COMPLETION | SL_RAIL_EVENT_IEEE802154_DATA_REQUEST_COMMAND |
	...
	if (IS_ENABLED(CONFIG_SILABS_SISDK_RAIL_MULTIPROTOCOL)) {
		events |= SL_RAIL_EVENT_SCHEDULER_STATUS;
	}

	status = sl_rail_config_events(data->radio_data.rail_handle, SL_RAIL_EVENTS_ALL, events);
	__ASSERT_NO_MSG(status == SL_RAIL_STATUS_NO_ERROR);
	...
	if (!IS_ENABLED(CONFIG_OPENTHREAD_THREAD_VERSION_1_1)) {
		...
		sl_rail_ieee802154_set_rx_to_enh_ack_tx(data->radio_data.rail_handle,
							&rx_to_enh_ack_tx);
	}
	...
	__ASSERT_NO_MSG(silabs_efr32_set_txpower(dev, 0) == 0);
	...
	sl_rail_config_rx_options(data->radio_data.rail_handle,
				  SL_RAIL_RX_OPTION_TRACK_ABORTED_FRAMES,
				  SL_RAIL_RX_OPTION_TRACK_ABORTED_FRAMES);
	__ASSERT_NO_MSG(status == SL_RAIL_STATUS_NO_ERROR);

	sl_rail_ieee802154_config_cca_mode(data->radio_data.rail_handle,
...
static void silabs_efr32_rail_timer_callback(struct sl_rail_multi_timer *tmr,
					     sl_rail_time_t expected_time, void *callback_arg);

static uint16_t get_symbol_duration_us(sl_rail_handle_t handle)
{
	...
	return (symbol_rate > 0) ? (uint16_t)symbol_rate : 1;
}

static sl_rail_status_t schedule_next_reading(sl_rail_handle_t handle,
...
static void energy_scan_finalize(sl_rail_handle_t handle)
{
	const sl_rail_config_t *rail_config = sl_rail_get_config(handle);
	const struct device *dev = rail_config->p_opaque_handle_0;
	struct silabs_efr32_802154_data *data = dev->data;
	...
	energy_scan_cleanup(handle, data);
					     ...
					     sl_rail_time_t expected_time, void *callback_arg)
{
	...
	if (ed_scan_data->frame_counter < ed_scan_data->frame_counter_max) {
		if (schedule_next_reading(handle, &data->radio_data.rail_timer)) {
			energy_scan_finalize(handle);
		}
	} else {
		energy_scan_finalize(handle);
	}
...
static void silabs_efr32_iface_init(struct net_if *iface)
{
	...
	(void)hwinfo_get_device_id(eui64, SL_802154_EUI64_BYTES);

	net_if_set_link_addr(iface, eui64, SL_802154_EUI64_BYTES, NET_LINK_IEEE802154);
			   ...
			   struct net_pkt *pkt, struct net_buf *frag)
{
	...
	len = (uint8_t)(frag->len + SL_802154_CRC_BYTES); /* PSDU = MPDU + FCS */
	...
	(void)sl_rail_write_tx_fifo(data->radio_data.rail_handle, data->tx_buffer, len, false);
					  ...
					  energy_scan_done_cb_t done_cb)
{
	...
	status = schedule_next_reading(handle, &data->radio_data.rail_timer);
	if (status != SL_RAIL_STATUS_NO_ERROR) {
		energy_scan_cleanup(handle, data);
		return -EBUSY;
	}
						 ...
						 const struct ieee802154_config *config)
{
	...
	if (element_id == IEEE802154_HEADER_IE_ELEMENT_ID_VENDOR_SPECIFIC_IE &&
		...
		memcpy(&data->ack_ie.link_metrics_header_ie, config->ack_ie.header_ie,
